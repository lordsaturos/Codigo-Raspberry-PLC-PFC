...
errStatus gpioI2cSetup(void)
{
    ...
    else
    {
		...
		//iNTENTO DESACTIVAR EL I2C_CLKT
		I2C_CLKT = BSC_TOUT;			//DESACTIVA
        rtn = OK;
		printf("i2cSetup Todo ok\n"); //Info sobre el resultado positivo.
...
errStatus gpioI2cWriteData(const uint8_t * data, uint16_t dataLength)
{
	...
	//printf(dataLength); //Para saber con la longitud de entrada
    if (gI2cMap == NULL)
    {
        dbgPrint(DBG_INFO, "gI2cMap was NULL. Ensure gpioI2cSetup() was called successfully.");
        rtn = ERROR_NOT_INITIALISED;
    ...
    else
    {
        ...
        {
			printf("gpioI2cWriteData Todo ok\n"); //Info sobre el resultado positivo.					
            rtn = OK;
        }
...
errStatus gpioI2cReadData(uint8_t * buffer, uint16_t bytesToRead)
{
    ...
    else
    {
        ...
        {
			printf("gpioI2cReadData Todo ok\n"); //Info sobre el resultado positivo.					
            rtn = OK;
        }
